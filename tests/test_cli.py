...
def test_generate_data(tmp_path: Path) -> None:
    result = runner.invoke(
        app, ["generate-data", "--output", str(tmp_path), "--days", "1", "--seed", "42"]
    )
    assert result.exit_code == 0
...
def test_backtest(sample_data_dir: Path, tmp_path: Path) -> None:
    result = runner.invoke(
        app, ["backtest", "--data-dir", str(sample_data_dir), "--output", str(tmp_path)]
    )
    assert result.exit_code == 0
...
def test_paper_run(sample_data_dir: Path) -> None:
    result = runner.invoke(
        app, ["paper-run", "--data-dir", str(sample_data_dir), "--duration", "10"]
    )
    assert result.exit_code == 0
...
def test_report(sample_data_dir: Path, tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        app, ["report", str(metrics_file), "--output", str(tmp_path / "summary.md")]
    )
    assert result.exit_code == 0
